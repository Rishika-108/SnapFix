...
const registerWorker = async (req, res) => {
    try {
        ...
        const worker = await Worker.create({
            name, email, password: hashedPassword, phone, skills: skillsArray, role: 'gigworker',
        ...
        res.status(201).json({
            success: true, message: "User registered successfully",
            worker: {
                id: worker._id, name: worker.name, email: worker.email, phone: worker.phone,
...
const loginWorker = async (req, res) => {
    try {
        ...
        res.status(200).json({
            ...
            worker: {
                id: worker._id, name: worker.name, email: worker.email, skills: worker.skills,
...
const loginAdmin = async (req, res) => {
    try {
        ...
        if (!admin) {
            // Initial setup: Hash the environment password and save
        ...
        const token = jwt.sign({ id: admin._id, email, role: admin.role }, process.env.JWT_SECRET, { expiresIn: "1d" })
